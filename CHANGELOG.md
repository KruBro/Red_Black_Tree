...
- `delete_t()` — BST delete with transplant, in-order successor handling, and `delete_fix_up()`
...
<!-- Maintainer note: when cutting a release, move items from [Unreleased] to a new
     version block above, date it, and tag the commit: git tag -a v1.x.x -m "vX.X.X" -->